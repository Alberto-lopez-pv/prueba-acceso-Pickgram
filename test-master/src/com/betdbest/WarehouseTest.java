...
public class WarehouseTest {
  ...
  static class ShipmentsManager {
    ...
    private ShipmentInfo findBestRoute(List<Warehouse> warehouses, Order order, BoxType box)
    	...
    	float lowestVolumePricing = 0;
    	ShippingHour bestShippingHour = null;
    	CarrierTime time = null;
    	Warehouse bestWarehouse = null;
    	LocalDateTime deleveryTime = null;
    	
    	int stock = 0;
    	
    	
    	for(int i=0; i<warehouses.size(); i++) {
    		//PARA SABER EL MEJOR PRECIO DEPENDIENDO DEL ALMACÉN QUE SALGAN LAS ÓRDENES
    		for(CarrierPricing p : pricings) {
    			if(p.getWarehouse().equals(warehouses.get(i))) {
    				if(p.getTargetState().equals(order.getTargetState())) {
    					if(i==0) {
    						lowestVolumePricing=p.volumePrice;
    						bestWarehouse=warehouses.get(i);
    					}	
    					else if(p.volumePrice < lowestVolumePricing) {
    						lowestVolumePricing=p.volumePrice;
    						bestWarehouse=warehouses.get(i);
    					}	
    					else if(p.volumePrice == lowestVolumePricing) {
    						for(Stock s : stocks) {
    							if(s.getItemId().equals(order.getItemId())) {
    								for(int j=0; j<warehouses.size(); j++) {
    									if(s.getWarehouse().equals(warehouses.get(j))) {
    										if(i==0) {
    											stock=s.getStock();
    											bestWarehouse=warehouses.get(j);
    										}
    										else if(s.getStock() > stock) {
    											stock=s.getStock();
    											bestWarehouse=warehouses.get(i);
    										}
    									}
    		...
    		//ANÁLISIS DEL TIEMPO DE SALIDA
    		for(DepartureTime dt: departures) {
    			if(dt.getWarehouse().equals(warehouses.get(i))){
    				if(dt.getTargetState().equals(order.getTargetState())) {
    					for(ShippingHour e: dt.getShippingHours()) {
    						bestShippingHour=new ShippingHour(e.getDay(), e.getTime());
    					}
    				}
    			}
    		}
    		//CÁLCULO DE TIEMPO DE ENTREGA
    		for(CarrierTime t : times) {
    			if(t.getWarehouse().equals(warehouses.get(i))){
    				if(t.getTargetState().equals(order.getTargetState())) {
    					time=t;
    				}
    			}
    		}
    	...
    	deleveryTime = getDeliveryDateTime (order.getOrderDate(), bestShippingHour, time);
    	info = new ShipmentInfo( order, bestWarehouse, deleveryTime, box.getBoxType(), (lowestVolumePricing*box.getVolume()));
    	return info;
    }
    ...
    private BoxType findBestBoxType(Order order) throws NoSuitableBoxException {
    	...
    	for(Item o: items) {
    		if(order.getItemId().equals(o.getItemId())){
    			for(BoxType b : boxTypes) {
    				if((b.getHeight() > o.getHeight()) && (b.getLength() > o.getLength()) && (b.getMaxWeight() > o.getWeight()) && (b.getWidth() > o.getWidth())) {
    					boxType=b;
    				}
    				else if(((o.getHeight()*o.getLength()*o.getHeight())/1000)<b.getVolume())
    					boxType=b;
    			} 
    		}
    	}
